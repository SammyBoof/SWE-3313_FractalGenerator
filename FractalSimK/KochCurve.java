...
public class KochCurve extends PaintedPanel implements Runnable{
    ...
    //declare attributes
    private static ArrayList<line> currentGen;
    ...
    public KochCurve(int i){
        ...
        int size;
        if(FractalGUI.isFullScreen) size = FULL_SIZE;
        else size = DEF_SIZE;
    ...
    public void drawFractal(Graphics2D g) {
        for(line l: currentGen){
            if(l.getType() == 1) g.setColor(Color.RED);
            if(l.getType() == 2) g.setColor(new Color(255,85,0));
            if(l.getType() == 3) g.setColor(Color.YELLOW);
            if(l.getType() == 4) g.setColor(new Color(0,255,85));
            if(l.getType() == 5) g.setColor(Color.BLUE);
